...
import { X, CalendarDays, Clock, Download, User, Mail, Phone, BookOpen, CheckCircle, FileText } from "lucide-react";
...
const MentorshipViewModal = ({ session, onClose, mentorsList, onUpdate }) => {
  ...
  useEffect(() => {
    if (session) {
      const normalized = {
        ...
        student: session.student || session.name || "Unknown",
        email: session.email || "",
  ...
  const formatDateForBackend = (inputDate) => {
    ...
    if (inputDate.includes("-") && inputDate.split("-").length === 3) {
      const parts = inputDate.split("-");
      if (parts[0].length <= 2) {
        return inputDate; // Already DD-MM-YYYY
      }
    }
    
    // Convert formatted date to DD-MM-YYYY
    try {
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          >
            <div>
              <h2 className="text-xl font-semibold">Mentorship Session Details</h2>
              <p className="text-sm text-white/90 mt-1">View and update mentorship session</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="overflow-y-auto p-6 space-y-6">
            ...
            <div className="bg-white p-5 border border-gray-200 rounded-xl shadow-sm">
              ...
              <textarea
                ...
                placeholder="Enter student's query..."
            ...
            <div className="bg-white p-5 border border-gray-200 rounded-xl shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <SelectField
                  ...
                  options={mentorsList || ["No mentors available"]}
                />
...
const InputField = ({ label, value, onChange, placeholder, icon, className = "" }) => (
