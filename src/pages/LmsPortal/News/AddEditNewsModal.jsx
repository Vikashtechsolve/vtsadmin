...
import { createNews, updateNews, fetchNewsById } from "../API/newsApi";

export default function AddEditNewsModal({ news, onClose }) {
  ...
  const loadNewsData = async () => {
    try {
      setLoadingData(true);
      const newsData = await fetchNewsById(news._id);
      if (newsData) {
        setFormData({
      ...
      setErrors({ submit: error.message || "Failed to load news data" });
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.image) {
        ...
        newsData.imageUrl = formData.imageUrl.trim();
      }

      if (isEditing) {
        await updateNews(news._id, newsData);
      } else {
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
              placeholder="e.g., AI, Technology, Updates"
            />
